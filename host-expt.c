...
This version was patched by Brian E Carpenter on 2021-10-02
to support draft-carpenter-6man-rfc6874bis. A couple of statements
...
/* str points to the IPv6 literal to be checked, i.e. one character after "[" */
/* end points to the remnant of the URL, i.e. to the "]" */

{
  ...
  const char *curtok;  /* pointer to start of current token in the literal */
  int tp;              /* number of address bytes encountered so far */
  bool colonp;         /* did we see ::? */
  bool saw_xdigit;     /* did we see any digits in this chunk? */
  unsigned int val;    /* the address chunk as a binary value */

  tp = 0;              /* no bytes so far */
  colonp = false;      /* no double colon so far */

  if (str == end)
    return false;      /* the literal is void */

  /* Leading :: requires some special handling. */
  if (*str == ':')
    {
      ++str;                           /* now pointing to 2nd character in literal */
      if (str == end || *str != ':')
        return false;                  /* cannot start with a single colon */    
    }

  curtok = str;         /* point to start of first chunk (token) */
                        /* (2nd colon if literal starts with ::) */
  saw_xdigit = false;   /* have not yet seen any digit */
  val = 0;              /* initialise binary value of address chunk */

  while (str < end)     /* step through the chunk */
    {
      int ch = *str++;
      ...
      if (c_isxdigit (ch))
        {
          val <<= 4;            /* shift left one nibble */
          val |= _unhex (ch);   /* OR in the new nibble */
          if (val > 0xffff)
            return false;         /* can't exceed 4 nibbles */
          saw_xdigit = true;
      ...
      /* if ch is a colon ... */
      if (ch == ':')
        {                      /* we're at the end of a chunk */
          curtok = str;        /* point to character after colon */
          if (!saw_xdigit)     /* if we have not just processed digit(s) */
            {
              if (colonp != false)  /* and we already saw :: */
                return false;         /* second :: */
              colonp = true;        /* now we've seen :: */
              continue;             /* goto next character */
            }
          else if (str == end)
            return false;           /* premature end */
          if (tp > ns_in6addrsz - ns_int16sz)
            return false;           /* too much address */
          tp += ns_int16sz;       /* finished a chunk (2 more bytes) */
          saw_xdigit = false;     /* initialize for next chunk */
          val = 0;
          continue;               /* goto next character */
        }
      ...
      if (ch == '.' && (tp <= ns_in6addrsz - ns_inaddrsz)
        ...
        {
          tp += ns_inaddrsz;     /* 4 more bytes */
          saw_xdigit = false;
          break;     /* exit loop after IPv4 chunk */
        }

      /* if ch is % */
      else if ((ch == '%') || (ch == '-'))
        {break;}     /* exit loop on % or - sign (cannot coexist with IPv4 address) */

      return false;  /* failure in IPv4 chunk */
    }

  if (saw_xdigit)    /* exited loop having just seen digit(s) */
    {
      if (tp > ns_in6addrsz - ns_int16sz)  
        return false;        /* too many bytes */         
      tp += ns_int16sz;      /* count the final chunk */
    }

  if (colonp != false)
    {                        /* we saw :: */
      if (tp == ns_in6addrsz)
        return false;        /* but already had enough bytes */
      tp = ns_in6addrsz;     /* must have been a double colon at the end */
    }

  if (tp != ns_in6addrsz)
    return false;            /* wrong number of bytes */

  return true;               /* correct number of bytes */
}
...
#endif
      if (numeric_address)
        /* Address literal, do we need to change a Zone ID separator from - to % ? */
        {
          char *zid;
          int zlen = strlen(host);
          char *zhost = malloc((zlen+1)*sizeof(char));
          int zindex;
          strcpy(zhost,host);
          zid = strchr(zhost, '-');
          if (zid)
            /* Yes, must change to RFC 4007 format */
            {            
              zindex = (int)(zid-zhost);
              zhost[zindex] = '%'; 
